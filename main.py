...
Owner = "Harshu XD"
repo = "https://github.com/harshdemon/Banall-Bot"
@main.on(events.NewMessage(pattern="^/start"))
async def start(event):
    buttns = [Button.url("ꜱᴜᴩᴩᴏʀᴛ", "https://t.me/wrostdemon"), Button.url("Sᴏᴜʀᴄᴇ ❤️", f'{repo}')]
    py = platform.python_version()
    if event.sender.id in SEXY:
        await main.send_file(
            event.chat.id,
            file="https://telegra.ph/file/52461a8eea41c9ee43f53.jpg",
    ...
    if event.sender.id not in SEXY:
        await main.send_file(
            event.chat.id,
            file="https://te.legra.ph/file/ae14f293ef4a9352c5248.jpg",
...
async def start(event):
    buttns = [Button.url("Sᴜᴘᴘᴏʀᴛ", "https://t.me/wrostdemon"), Button.url("Sᴏᴜʀᴄᴇ ❤️", f'{repo}')]
    py = platform.python_version()
    if event.sender.id in SEXY:
        await main.send_file(
            event.chat.id,
    ...
    if event.sender.id not in SEXY:
        await event.reply(
            "Tʜɪs ɪs ɴᴏᴛ ғᴏʀ ʏᴏᴜ ʙᴀʙᴇ!\n\nᴍᴀᴋᴇ yᴏᴜʀ ᴏᴡɴ ʙᴏᴛ ꜰʀᴏᴍ ᴛʜɪꜱ [Repository](https://github.com/harshdemon/Banall-Bot)",
            link_preview=False,
        )       
...
print("Join @Wrostdemon if you facing any kind of issue!!")
